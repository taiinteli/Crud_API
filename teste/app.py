...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///data/mydatabase.db'  # Use o SQLite para simplificar
...
def create_user():
    if not User.query.filter_by(id='teste').first():
        user = User(id='teste', password='teste123')
        db.session.add(user)
        db.session.commit()
